...
const Predict = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    setResult(null);
    console.log('Submitting prediction request with data:', formData); // Log the request data
    try {
      ...
      console.error('Prediction error:', error); // Already present, but ensure it's visible
      if (error.code === 'ECONNABORTED') {
        setError('Request timed out. The server might be starting up. Please try again in a few seconds.');
      } else if (error.response) {
        console.log('Error response from server:', error.response.data); // Log server error details
  ...
  const handleDownload = () => {
    if (predictionId) {
      console.log('Downloading report for prediction ID:', predictionId); // Log download action
